...
namespace radzki.DileraHornMod.util;


internal class AudioUtility
{
    /// <summary>
        ...
        using (UnityWebRequest uwr = UnityWebRequestMultimedia.GetAudioClip(path, type))
        {
            ...
            try
            {
                ...
                if (uwr.result != UnityWebRequest.Result.Success)
                    Plugin.Log.LogError($"Failed to load WAV AudioClip from path: {path} Full error: {uwr.error}");
                else
                {
            ...
            {
                Plugin.Log.LogError($"{err.Message}, {err.StackTrace}");
            }
        }
